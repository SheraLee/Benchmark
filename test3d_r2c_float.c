...
main(int argc,char **argv)
{
  ...
  FIN= (float *) malloc(sizeof(float) *size1);
...
void init_wave(float *IN,int *gdims,int *ldims,int *gstart)
{
  float *sinx,*siny,*sinz,sinyz,*p;
  int x,y,z;
  float twopi = atan(1.0)*8.0;

  sinx = (float *) malloc(sizeof(float)*gdims[0]);
  siny = (float *) malloc(sizeof(float)*gdims[1]);
  sinz = (float *) malloc(sizeof(float)*gdims[2]);
...
void print_res(float *A,int *gdims,int *ldims,int *gstart)
{
  int x,y,z;
  float *p;
  float Nglob;
...
float check_res(float *A,float *B,int *ldims)
{
  int x,y,z;
  float *p1,*p2,mydiff;
  int imo[3],i,j;
